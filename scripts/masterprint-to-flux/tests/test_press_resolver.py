...
from press_resolver import _press_nusec, _press_minutes_from_launches, resolve_press
...
def _launch(numdo: str = "202604.0001/A", nusec: str = "G37", tps: list[float] | None = None) -> Launch:
    return Launch(
...
# ============ _press_minutes_from_launches ============

def test_press_minutes_sums_only_matching_indices():
    """For G37 (NUSEC_4 and NUSEC_10 = G37), only TPSALLOUE_4 + TPSALLOUE_10 count."""
    m = _machine("G37", "G37", ["0", "0", "CTP-2", "G37", "CTP-2", "0", "0", "0", "0", "G37", "0", "0", "0", "0"])
    launch = _launch(tps=[0.2, 0, 0.56, 0.1, 2.49, 0, 0, 0, 0, 3.37])  # _3=33.6min (CTP), _5=149min (CTP)
    # Indices 4 and 10 sum: 0.1h + 3.37h = 3.47h = 208.2 → 208 min
    assert _press_minutes_from_launches(m, [launch], "G37") == 208


def test_press_minutes_aggregates_multiple_launches():
    """Several Launch rows (e.g. /A and /B variants) get summed together."""
    m = _machine("G37", "G37", ["0"] * 3 + ["G37"] + ["0"] * 10)
    launches = [
        _launch(tps=[0, 0, 0, 0.5] + [0] * 6),   # 30 min
        _launch(numdo="202604.0001/B", tps=[0, 0, 0, 0.25] + [0] * 6),  # 15 min
    ]
    assert _press_minutes_from_launches(m, launches, "G37") == 45


def test_press_minutes_zero_when_no_launches():
    m = _machine("G37", "G37", ["G37"] * 14)
    assert _press_minutes_from_launches(m, [], "G37") == 0
...
def test_resolve_press_returns_none_when_cdmac_empty():
    ...
    m = _machine("G37", "G37", ["G37"] * 14)
    assert resolve_press(imp, "202604.0001", m, [_launch(tps=[1.0] + [0]*9)], {}) is None
...
def test_resolve_press_returns_none_when_total_zero():
    """Launch existe mais toutes les durées sont 0."""
    imp = _impression(cdmac_1="G37")
    m = _machine("G37", "G37", ["G37"] * 14)
    launch = _launch(tps=[0.0] * 10)
...
def test_resolve_press_maps_via_yaml_press_mapping():
    ...
    launch = _launch(nusec="RICOH", tps=[0.5] + [0]*9)
    ...
    assert res.setup_min_override == 0
    assert res.run_min_override == 30
...
def test_resolve_press_falls_back_to_libma_when_no_mapping():
    ...
    launch = _launch(nusec="UNKNOWN", tps=[1.0] + [0]*9)
    res = resolve_press(imp, "202604.0001", m, [launch], {})
    assert res is not None
    assert res.station_name == "Some Press"


def test_resolve_press_collapses_to_zero_setup_force_format():
    """Press tasks sortent toujours en 0+X comme MASSI (force_setup_run_format)."""
    imp = _impression(cdmac_1="G37")
    m = _machine("G37", "G37", ["G37"] * 14)
    launch = _launch(tps=[0.5] + [0]*9)
    res = resolve_press(imp, "202604.0001", m, [launch], {})
    assert res is not None
    assert res.setup_min_override == 0
    assert res.run_min_override == 30

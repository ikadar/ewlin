#!/bin/bash
# Flux Scheduler - Deploy Push for LGI instance (run from local machine)
#
# Usage:
#   ./scripts/deploy-push-lgi.sh                      Deploy current state
#   ./scripts/deploy-push-lgi.sh snapshot.json         Restore to snapshot first, then deploy
#   ./scripts/deploy-push-lgi.sh --dry-run             Show what would happen
#   ./scripts/deploy-push-lgi.sh --dry-run snap.json   Dry-run with snapshot restore
...
REMOTE_DIR="/home/ordo/ordo-lgi"

echo "=========================================="
echo "Flux Scheduler - Deploy Push [LGI]"

/**
 * ReferentsPage - CRUD page for referents (contact persons)
 *
 * Accessible at /settings/referents.
...
import type { ReferentResponse } from '../store/api/referentApi';

// ============================================================================
// Referent Form Modal
// ============================================================================

interface ReferentFormModalProps {
  initial?: ReferentResponse | null;
  onSave: (data: { name: string }) => Promise<void>;
  onCancel: () => void;
  isSaving: boolean;
}

function ReferentFormModal({ initial, onSave, onCancel, isSaving }: ReferentFormModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onCancel} onKeyDown={e => { if (e.key === 'Escape') onCancel(); }} tabIndex={-1}>
      <div className="bg-flux-elevated border border-flux-border-light rounded-lg p-6 w-full max-w-sm mx-4 shadow-xl" onClick={e => e.stopPropagation()}>
        <h2 className="text-flux-text-primary font-medium mb-4">
          {initial ? 'Modifier le référent' : 'Nouveau référent'}
        </h2>
...
export function ReferentsPage() {
  ...
  const [editingReferent, setEditingReferent] = useState<ReferentResponse | null>(null);
  const [isCreating, setIsCreating] = useState(false);
  const [deletingReferent, setDeletingReferent] = useState<ReferentResponse | null>(null);
  const [deleteError, setDeleteError] = useState<string | null>(null);
  const [saveError, setSaveError] = useState<string | null>(null);

  const { data: referents = [], isLoading, error } = useGetReferentsQuery();
  const [createReferent, { isLoading: isCreatingLoading }] = useCreateReferentMutation();
  const [updateReferent, { isLoading: isUpdatingLoading }] = useUpdateReferentMutation();
  const [deleteReferent] = useDeleteReferentMutation();

  // Keyboard shortcuts
  useEffect(() => {
    if (editingReferent || isCreating || deletingReferent) return;
    ...
    document.addEventListener('keydown', handleKeyDown);
    return () => document.removeEventListener('keydown', handleKeyDown);
  }, [navigate, editingReferent, isCreating, deletingReferent]);

  // Client-side filtering
  const filteredReferents = useMemo(() => {
    if (!searchQuery.trim()) return referents;
    const query = searchQuery.toLowerCase();
    return referents.filter((r) => r.name.toLowerCase().includes(query));
  }, [referents, searchQuery]);

  // Handlers
  const handleSaveCreate = async (data: { name: string }) => {
    setSaveError(null);
    try {
      await createReferent({ name: data.name }).unwrap();
      setIsCreating(false);
    } catch (err: unknown) {
      const msg =
  ...
  const handleSaveEdit = async (data: { name: string }) => {
    ...
    setSaveError(null);
    try {
      await updateReferent({ id: editingReferent.id, body: { name: data.name } }).unwrap();
  ...
  const handleDeleteConfirm = async () => {
    ...
    setDeleteError(null);
    try {
      await deleteReferent(deletingReferent.id).unwrap();
    ...
    <div className="min-h-screen bg-flux-base flex flex-col">
      ...
      <header className="border-b border-flux-border px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl font-semibold text-flux-text-primary">Référents</h1>
        </div>
        <button
      ...
      <main className="flex-1 p-6">
        ...
        {error && (
          <div className="text-center text-red-400 mt-20">
            Erreur de chargement des référents
          </div>
        )}
            ...
            <div className="mb-4 flex items-center gap-4">
              <div className="relative flex-1 max-w-md">
                ...
                <input
                  ...
                  aria-label="Rechercher un référent"
                  value={searchQuery}
              ...
              <span className="text-flux-text-tertiary text-sm">
                {filteredReferents.length} référent
                {filteredReferents.length !== 1 ? 's' : ''}
                {searchQuery && ` / ${referents.length}`}
              </span>
            </div>
              ...
              <table className={FLUX_TABLE}>
                ...
                <tbody>
                  ...
                  {filteredReferents.map((referent) => (
                    <tr key={referent.id} className={FLUX_BODY_TR} style={FLUX_BODY_TR_STYLE}>
                      <td className={FLUX_BODY_CELL_PRIMARY}>{referent.name}</td>
                      <td className={FLUX_BODY_CELL}>
                        {new Date(referent.createdAt).toLocaleDateString('fr-FR')}
                      </td>
                      <td className={FLUX_BODY_CELL}>
                        <FluxRowActions
                          onEdit={() => { setSaveError(null); setEditingReferent(referent); }}
                          onDelete={() => { setDeleteError(null); setDeletingReferent(referent); }}
                        />
                      </td>
                    </tr>
                  ))}
      ...
      {editingReferent && (
        <ReferentFormModal
          initial={editingReferent}
          onSave={handleSaveEdit}
          onCancel={() => { setEditingReferent(null); setSaveError(null); }}
          isSaving={isUpdatingLoading}
        />
      )}
      {editingReferent && saveError && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[60] px-4 py-1.5 bg-red-900/80 border border-red-500/40 rounded text-sm text-red-300">
          {saveError}
        </div>
      )}
      ...
      {/* Delete confirmation */}
      {deletingReferent && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => setDeletingReferent(null)} onKeyDown={e => { if (e.key === 'Escape') setDeletingReferent(null); }} tabIndex={-1}>
          <div className="bg-flux-elevated border border-flux-border-light rounded-lg p-6 max-w-sm w-full mx-4 shadow-xl" onClick={e => e.stopPropagation()}>
            <h2 className="text-flux-text-primary font-medium mb-2">Supprimer le référent</h2>
            <p className="text-sm text-flux-text-secondary mb-4">
              Supprimer{' '}
              <span className="font-medium text-flux-text-primary">{deletingReferent.name}</span> ?
            ...
            <div className="flex gap-3 justify-end">
              <button
                onClick={() => { setDeletingReferent(null); setDeleteError(null); }}
                className="px-3 py-1.5 text-sm text-flux-text-secondary hover:text-flux-text-primary bg-flux-active hover:bg-flux-hover rounded transition-colors"
              >
                Annuler
              </button>

/**
 * FormeLeadTimePage — admin UI for the global Forme (die) Lead Time rule.
 *
 * Mirrors PaperLeadTimePage one-to-one with a distinct config row.
 * Accessible at /settings/forme-lead-time. Triggers Snapshot invalidation
 * so the next compute applies the new rule.
 */

import { useEffect, useState } from 'react';
import { Scissors, Check } from 'lucide-react';
import { useGetFormeLeadTimeQuery, useUpdateFormeLeadTimeMutation } from '../store';
...
export function FormeLeadTimePage() {
  const { data: config, isLoading } = useGetFormeLeadTimeQuery();
  const [updateFormeLeadTime, { isLoading: isSaving }] = useUpdateFormeLeadTimeMutation();
  ...
  const handleSave = async () => {
    ...
    try {
      await updateFormeLeadTime(draft).unwrap();
      setSavedAck(true);
      setTimeout(() => setSavedAck(false), 1800);
    } catch (err) {
      console.error('Failed to update forme lead time', err);
    }
  };
      ...
      <div className="max-w-2xl mx-auto p-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-lg font-semibold text-flux-text-primary">
            Délai d'approvisionnement forme
          </h1>
        </div>
        <p className="text-sm text-flux-text-tertiary mb-8 leading-relaxed">
          Règle utilisée par le moteur pour gater les éléments dont la forme
          (outil de découpe) n'est pas encore disponible (statuts <em>à
          commander</em> ou <em>commandée</em>). Le démarrage est repoussé à
          la première date à laquelle le fournisseur garantit la livraison
          de la forme, en jours ouvrés.
        </p>
        ...
        {isLoading ? (
          ...
          <div className="bg-flux-surface border border-flux-border rounded-lg p-6">
            <div className="flex items-baseline justify-between py-3">
              <label htmlFor="forme-cutoff" className="text-sm text-flux-text-secondary">
            ...
            <div className="flex items-baseline justify-between py-3 border-t border-flux-border">
              <label htmlFor="forme-offset" className="text-sm text-flux-text-secondary">
                Délai de livraison
              </label>
              <div className="flex items-baseline gap-2">
                <input
            ...
            <div className="flex items-baseline justify-between py-3 border-t border-flux-border">
              <label htmlFor="forme-arrival" className="text-sm text-flux-text-secondary">
        ...
        <div className="mt-8 space-y-5 text-sm text-flux-text-secondary leading-relaxed">
          ...
          <section>
            ...
            <p>
              ...
              que sa forme de découpe n'est pas physiquement présente. Cette
              règle traduit la promesse du fournisseur de formes en une{' '}
              <strong>date plancher</strong> par élément&nbsp;: la référence
              prise en compte est la date de commande si le statut est{' '}
              <em>commandée</em>, ou maintenant si le statut est{' '}
              <em>à commander</em>.
            </p>
          </section>
          ...
          <section>
            ...
            <ul className="space-y-2 list-disc pl-5">
              <li>
                ...
                duquel le fournisseur de formes traite la commande le lendemain
                seulement.
              </li>
              <li>
                <strong>Délai de livraison</strong>&nbsp;— combien de jours ouvrés
                après le jour de traitement la forme arrive. Les formes sont
                souvent plus longues que le papier (5 à 10 jours ouvrés selon
                le fournisseur et la complexité de la découpe).
              </li>
              <li>
                <strong>Heure de livraison maximale</strong>&nbsp;— l'heure tardive
                à laquelle on est sûr d'avoir la forme le jour de livraison.
              </li>
            </ul>
          </section>

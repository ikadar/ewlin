/**
 * ImpressionPresetsPage - CRUD page for impression presets
 *
 * Accessible at /impression-presets.
 * Follows the same pattern as FormatsPage and ClientsPage.
 */
...
} from '../store/api/impressionPresetApi';
import type { ImpressionPresetResponse } from '../store/api/impressionPresetApi';
import { toPrettyImpression, isValidImpression } from '../components/JcfImpressionAutocomplete/impressionDsl';

// ============================================================================
// Impression Preset Form Modal
// ============================================================================

interface ImpressionPresetFormModalProps {
  initial?: ImpressionPresetResponse | null;
  presets: ImpressionPresetResponse[];
...
function ImpressionPresetFormModal({
  initial,
  presets,
  onSave,
  onCancel,
  isSaving,
  saveError,
}: ImpressionPresetFormModalProps) {
  ...
  const handleValueChange = (v: string) => {
    ...
    if (!labelEdited) {
      const pretty = isValidImpression(v.trim()) ? toPrettyImpression(v.trim()) : '';
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div>
            ...
            <input
              ...
              maxLength={30}
              value={value}
              list="impression-preset-suggestions"
              ...
              placeholder="Ex: Q/Q, N/N, Q+V/"
            />
            <datalist id="impression-preset-suggestions">
              {suggestions.map((s) => (
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 bg-flux-base border border-flux-border-light rounded text-flux-text-primary placeholder:text-flux-text-muted focus:outline-none focus:border-flux-text-secondary transition-colors"
              placeholder="Ex: quadri recto/verso"
            />
          </div>
...
// ============================================================================
// ImpressionPresetsPage
// ============================================================================

export function ImpressionPresetsPage() {
  const navigate = useNavigate();
  const [searchQuery, setSearchQuery] = useState('');
  const searchInputRef = useRef<HTMLInputElement>(null);

  const [editingPreset, setEditingPreset] = useState<ImpressionPresetResponse | null>(null);
  const [isCreating, setIsCreating] = useState(false);
  const [deletingPreset, setDeletingPreset] = useState<ImpressionPresetResponse | null>(null);
  const [deleteError, setDeleteError] = useState<string | null>(null);
  const [saveError, setSaveError] = useState<string | null>(null);

  const { data: presets = [], isLoading, error } = useGetImpressionPresetsQuery();
  const [createPreset, { isLoading: isCreatingLoading }] = useCreateImpressionPresetMutation();
  const [updatePreset, { isLoading: isUpdatingLoading }] = useUpdateImpressionPresetMutation();
  const [deletePreset] = useDeleteImpressionPresetMutation();
    ...
    <div className="min-h-screen bg-flux-base flex flex-col">
      ...
      <header className="border-b border-flux-border px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl font-semibold text-flux-text-primary">Impressions</h1>
        </div>
        <button
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  ...
                  {filteredPresets.map((preset) => {
                    const effectiveLabel = preset.label || toPrettyImpression(preset.value);

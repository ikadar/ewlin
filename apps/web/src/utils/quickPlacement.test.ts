...
import type { Job, Task, TaskAssignment, InternalTask, Element } from '@flux/types';
...
describe('getLastUnscheduledTask', () => {
  ...
  it('returns the highest sequence unscheduled task', () => {
    const tasks: Task[] = [
      createInternalTask('task-1', 'element-job-1', 'station-a', 1),
...
describe('getAvailableTaskForStation', () => {
  ...
  it('returns null when station does not have the last unscheduled task', () => {
    ...
    const resultB = getAvailableTaskForStation(job, tasks, elements, assignments, stationB);
  ...
  it('supports backward scheduling: place last task first', () => {
    ...
    expect(getAvailableTaskForStation(job, tasks, elements, assignments, stationA)).toBeNull();
    expect(getAvailableTaskForStation(job, tasks, elements, assignments, stationB)).toBeNull();
    expect(getAvailableTaskForStation(job, tasks, elements, assignments, stationC)?.id).toBe('task-3');
  });
  ...
  it('allows previous task after last is scheduled', () => {
    ...
    expect(getAvailableTaskForStation(job, tasks, elements, assignments, stationA)).toBeNull();
    expect(getAvailableTaskForStation(job, tasks, elements, assignments, stationB)?.id).toBe('task-2');
    expect(getAvailableTaskForStation(job, tasks, elements, assignments, stationC)).toBeNull();
  });
  ...
  it('works through full backward scheduling sequence', () => {
    ...
    expect(getAvailableTaskForStation(job, tasks, elements, assignments, stationA)?.id).toBe('task-1');
...
describe('getStationsWithAvailableTasks', () => {
  ...
  it('shifts to previous station as tasks are scheduled', () => {
    ...
    result = getStationsWithAvailableTasks(job, tasks, elements, [createAssignment('task-3', stationC)]);
    expect(result).toEqual([stationB]);

    // Task 2 & 3 scheduled → only stationA
    result = getStationsWithAvailableTasks(job, tasks, elements, [
      createAssignment('task-3', stationC),
      createAssignment('task-2', stationB),
    ]);

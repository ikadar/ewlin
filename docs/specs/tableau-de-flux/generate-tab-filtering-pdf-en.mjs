...
execSync('pandoc -s tab-filtering.md -o tab-filtering-tmp.html', { cwd: __dirname });

let html = readFileSync(resolve(__dirname, 'tab-filtering-tmp.html'), 'utf-8');
...
await page.pdf({
  path: resolve(__dirname, 'tab-filtering-en.pdf'),
  format: 'A4',
...
console.log('PDF generated: tab-filtering-en.pdf');
await browser.close();

// Clean up temp file
execSync('rm tab-filtering-tmp.html', { cwd: __dirname });

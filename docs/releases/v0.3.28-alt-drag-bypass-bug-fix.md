# Release v0.3.28 – Alt+Drag Bypass Bug Fix
...
1. The job appears in the "Problèmes" section
...
// currentValidation was run with bypassPrecedence: true → hasPrecedenceConflict = false!
```
...
- Job appears in "Problèmes" section after bypassed placement
...
- Job disappears from Problèmes section after valid reschedule
...
- [ ] Alt+drop on conflicting position → job appears in Problèmes section
- [ ] Alt+drop on non-conflicting position → no conflict added
- [ ] Regular drop (no Alt) with precedence conflict → auto-snaps, no conflict
...
- [ ] Job card in Problèmes shows amber background (`bg-amber-500/10`)
...
- **Alt+Drop Conflict Recording:** When users bypass precedence rules using Alt+drop,
  the conflict is now correctly saved. The job appears in the "Problèmes" section
...
2. Hold Alt and drop
3. Verify job appears in Problèmes section

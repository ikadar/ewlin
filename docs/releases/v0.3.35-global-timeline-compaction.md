...
- [ ] **Create compactTimeline utility function**
...
- [ ] `compactTimeline()` correctly identifies movable vs immobile tasks
- [ ] `compactTimeline()` respects precedence rules
- [ ] `compactTimeline()` only processes tasks within horizon
- [ ] `compactTimeline()` doesn't move tasks in progress
- [ ] TopNavBar renders compact buttons
- [ ] TopNavBar shows loading state during compaction
- [ ] Compact buttons trigger correct horizon callback

### E2E Tests

- [ ] Clicking 4h button removes gaps within 4-hour horizon
- [ ] Compaction respects precedence (no violations created)
- [ ] Loading state visible during compaction

# Release v0.3.30 – Job Deselection Methods
...
| Action | `onClose?.()` → `setSelectedJobId(null)` |
...
- [x] **Update JobsList to support toggle**
  - Description: Click selected job → deselect
...
- [x] Click close button → Job Details Panel disappears
- [x] Click selected job in Jobs List → Job Details Panel disappears
- [x] Click different job → Selection changes (not deselect)
...
- [x] Roadmap updated (mark v0.3.30 as complete)
- [x] Git tag created: `v0.3.30`
...
1. Select a job → Job Details Panel appears
2. Click X button → Panel disappears
3. Select same job again → Click it again → Panel disappears (toggle)

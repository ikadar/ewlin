# Release v0.3.27 – Drag-Drop E2E Test Suite
...
### UC-01: New Task Placement (Sidebar → Grid)
...
- [x] **EC-02: Drag Outside Column** ✅
  - Tests: `apps/web/playwright/edge-cases.spec.ts` (2 tests)

- [x] **EC-04: Multiple Tiles Push-Down Chain** ✅
  - Tests: `apps/web/playwright/edge-cases.spec.ts` (2 tests)
...
apps/web/playwright/
├── helpers/
│   └── drag.ts                    # Reusable drag helper functions
├── fixtures/
│   └── test-data.ts               # Deterministic test data factories
├── drag-drop.spec.ts              # Core reschedule tests (existing)
├── push-down.spec.ts              # Push-down collision tests
├── station-constraint.spec.ts     # Station validation tests
├── precedence.spec.ts             # Precedence validation tests
├── approval-gates.spec.ts         # BAT/Plates gate tests
├── visual-feedback.spec.ts        # Visual indicator tests
├── swap.spec.ts                   # Swap button tests
├── quick-placement.spec.ts        # Quick placement mode tests

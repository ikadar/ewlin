# Release v0.3.10 – Similarity Indicators
...
4. For each criterion:
   ...
   - Compare values:
     - If both null/undefined → matched (link icon)
     - If one null, one has value → not matched (unlink icon)
     - If equal → matched (link icon)
     - If different → not matched (unlink icon)
...
- [x] **SimilarityIndicators component**
...
- [x] **Category criteria generation**
...
- [x] SimilarityIndicators renders correct number of icons
- [x] Link icon shown for matched criterion
- [x] Unlink icon shown for non-matched criterion
- [x] Null/null comparison returns matched
- [x] Null/value comparison returns not matched
- [x] Field path resolution works for nested paths
- [x] No indicators shown for single tile
...
├── SimilarityIndicators.tsx      # New component
├── similarityUtils.ts            # New utilities
├── Tile.tsx                      # (existing)
├── SwapButtons.tsx               # (existing)
├── colorUtils.ts                 # (existing)
├── index.ts                      # Updated exports
└── Tile.test.tsx                 # Updated tests
```

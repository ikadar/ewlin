...
- Reschedule follows same validation rules as new placement (per UX spec)
...
- [x] **Reschedule validation**
  - Description: Reschedule follows same validation rules as new placement (auto-snap, Alt bypass)
  - Files: `apps/web/src/App.tsx`

- [x] **isRescheduleDrag state**
  - Description: Tracks whether current drag is reschedule vs new placement (for column collapse)
  - Files: `apps/web/src/App.tsx`
...
- [x] **Operating hours validation timezone fix**
  - Description: Use local time instead of UTC for operating hours comparison
  - Files: `packages/validator/src/validators/availability.ts`

### Precedence Auto-Snap

- [x] **Precedence conflict auto-snap fix**
  - Description: PrecedenceConflict with suggestedStart is not blocking - tile snaps to valid position
  - Files: `apps/web/src/App.tsx`

### Compact Precedence

- [x] **Compact respects precedence rules**
  - Description: Station compact uses task.sequenceOrder to find predecessors and respect their end times
  - Files: `apps/web/src/App.tsx`

### Approval Gate

- [x] **Plates ApprovalGateConflict is warning only**
  - Description: BAT is blocking, Plates is warning (not blocking)
  - Files: `apps/web/src/App.tsx`

- [x] **Orange warning color for non-blocking conflicts**
  - Description: Plates approval shows orange highlight instead of red (visual distinction between blocking and warning)
  - Files: `apps/web/src/hooks/useDropValidation.ts`, `apps/web/src/App.tsx`, `apps/web/src/components/SchedulingGrid/SchedulingGrid.tsx`, `apps/web/src/components/StationColumns/StationColumn.tsx`

### End Time Stretching

- [x] **End time stretching across non-operating periods (BR-ASSIGN-003b)**
  - Description: calculateEndTime stretches across operating slots, skipping downtime
  - Files: `apps/web/src/utils/timeCalculations.ts`, `apps/web/src/App.tsx`

- [x] **Compact uses calculateEndTime for stretching**
  - Description: Compact operation uses same stretching algorithm as manual placement
  - Files: `apps/web/src/App.tsx`

- [x] **Initial assignments use stretching**
  - Description: Mock assignment generator applies BR-ASSIGN-003b stretching
  - Files: `apps/web/src/mock/generators/assignments.ts`

### Validation Timing

- [x] **Real-time validation uses unsnapped position**
  - Description: Drag-over validation feedback matches cursor position instead of snapped position
  - Files: `apps/web/src/App.tsx`
...
- Reschedule now follows same validation rules as new placement (per UX spec)
...
- Fixed operating hours validation off by one hour (UTC vs local timezone)
- Fixed precedence conflict blocking drop instead of auto-snapping to valid position
- Fixed compact not respecting precedence rules (used task.sequenceOrder)
- Fixed Plates ApprovalGateConflict incorrectly blocking drops
- Added orange warning color for non-blocking conflicts (Plates approval)
- Implemented end time stretching across non-operating periods (BR-ASSIGN-003b)
- Fixed compact not using stretching algorithm
- Fixed initial assignments not using stretching algorithm
- Fixed validation feedback timing (30-minute offset during drag-over)

### Technical Changes

- Custom collision detection using `document.elementsFromPoint()`
- Added `pointer-events-none` to DragPreview component
- Added `isRescheduleDrag` state for column collapse control (reschedule uses same validation as new placement)
- Local compact implementation for mock mode
- Rebuilt @flux/schedule-validator package
- Use local time for operating hours validation instead of UTC
- PrecedenceConflict with suggestedStart not treated as blocking conflict
- Plates ApprovalGateConflict is warning only (BAT remains blocking)
- Orange highlight (`ring-orange-500`) for warning-only conflicts, red for blocking
- calculateEndTime stretches end time across station operating slots
- Compact and initial assignment generation use calculateEndTime for consistent stretching
- Real-time validation uses unsnapped cursor position for accurate visual feedback
```
...
apps/web/src/App.tsx
  ...
  - isRescheduleDrag state (for column collapse, not validation bypass)
  - MeasuringStrategy.Always
  - Reschedule uses same validation as new placement
  - Local compact implementation (mock mode)
  - Pass isRescheduleDrag to SchedulingGrid
  - handleDragMove uses unsnapped position for real-time validation

apps/web/src/components/SchedulingGrid/SchedulingGrid.tsx
  - isRescheduleDrag prop
  - Column collapse disabled during reschedule
  - hasWarningOnly in ValidationState
  - isWarningDrop calculation for orange highlight
...
apps/web/src/components/StationColumns/StationColumn.tsx
  - isWarningDrop prop for orange highlight
  - ring-orange-500 for warning-only conflicts

apps/web/src/hooks/useDropValidation.ts
  - hasWarningOnly calculation (non-blocking conflicts only)
...
apps/web/src/mock/generators/assignments.ts
  - Use calculateEndTime for BR-ASSIGN-003b stretching

packages/validator/src/validators/availability.ts
  - Use local time (getHours, getMinutes) instead of toISOString() UTC
  - Use local date (getFullYear, getMonth, getDate) for exception matching

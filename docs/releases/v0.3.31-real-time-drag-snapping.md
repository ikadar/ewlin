# Release v0.3.31 – Real-Time Drag Snapping
...
This release implements real-time drag preview snapping to the 30-minute grid during drag operations. Currently, the drag preview follows the cursor freely and only snaps to the grid position on drop. After this release, the preview will snap to grid positions in real-time during the drag, providing immediate visual feedback of where the tile will land.
...
The `DragLayer.tsx` component positions the preview at the raw cursor position:

```typescript
// Current (no snapping during drag)
...
The preview should snap to 30-minute grid positions during drag:
...
- [ ] **Apply snapToGrid to DragLayer preview position**
...
- [ ] **Import snapToGrid in DragLayer**
...
- [ ] **Verify horizontal movement restriction**
...
- [ ] Drag preview snaps to 30-minute positions during drag
- [ ] Drag preview vertical position changes in 40px increments
- [ ] Tile placement matches preview position after drop

### Manual Testing

- [ ] Start dev server: `pnpm dev`
- [ ] Load fixture: `http://localhost:5173/?fixture=test`
- [ ] Drag a task from sidebar
- [ ] Verify preview snaps to grid lines during drag (not just on drop)
- [ ] Verify horizontal position stays fixed
...
| Fixture | URL | Purpose |
|---------|-----|---------|
| `test` | `http://localhost:5173/?fixture=test` | Multiple tasks for drag testing |
...
**Expected Results:**
- [ ] Drag preview "jumps" between 30-minute grid positions
- [ ] Preview does NOT follow cursor smoothly between grid lines
- [ ] Preview position changes in discrete 40px steps
- [ ] Preview snaps to nearest grid line (not cursor position)
...
**Expected Results:**
- [ ] Tile lands at the snapped position (matching preview)
- [ ] No difference between preview position and final tile position
...
**Expected Results:**
- [ ] Preview stays in the target column
- [ ] Preview does not follow horizontal cursor movement
- [ ] Only vertical movement affects preview position
...
- [ ] Roadmap updated (mark v0.3.31 as complete)
- [ ] Git tag created: `v0.3.31`

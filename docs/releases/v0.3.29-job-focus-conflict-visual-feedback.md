# Release v0.3.29 – Job Focus & Conflict Visual Feedback
...
- [x] Select job → non-selected job tiles are muted
- [ ] Deselect job → all tiles return to normal (skipped - REQ-02/03 not implemented yet)
- [x] Create precedence conflict via Alt+drop → tile shows amber glow
- [x] Move conflicting task to valid position → amber glow disappears
- [x] Select job with conflict tile → amber glow (not job color glow)
...
- [ ] Select a different job → muting updates to show new selection
...
1. Select a job → other job tiles become muted
2. Create conflict via Alt+drop → tile shows amber glow
3. Move tile to valid position → amber glow disappears

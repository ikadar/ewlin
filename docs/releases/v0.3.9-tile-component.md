...
<div class="absolute left-0 right-0 pt-0.5 px-2 text-sm border-l-4 border-l-rose-500 bg-rose-950/35" style="top: 280px; height: 160px;">
  <div class="flex items-center gap-2">
    ...
    <span class="text-rose-300 font-medium truncate">22342 · BNP Paribas</span>
  </div>
</div>
```
...
- [x] **Single click - select job**
  - Description: Select job when tile is clicked
  - Files: `apps/web/src/components/Tile/Tile.tsx`

- [x] **Double click - recall tile**
...
- [x] Tile renders job reference and client
- [x] Tile displays correct completion icon
- [x] Completed tile shows green gradient
- [x] Setup section has correct height
- [x] Run section has correct height
- [x] Swap buttons visible on hover
- [x] Single click calls onSelect
- [x] Double click calls onRecall
- [x] Color mapping returns correct classes
...
├── Tile/
│   ├── index.ts           # Exports
│   ├── Tile.tsx           # Main component
│   ├── Tile.test.tsx      # Tests
│   ├── SwapButtons.tsx    # Swap buttons
│   └── colorUtils.ts      # Color mapping
├── SchedulingGrid/
│   └── SchedulingGrid.tsx # Updated with tiles

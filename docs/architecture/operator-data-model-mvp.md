 ...
 *
 * Records an `OperatorAssignmentChanged` domain event (published to Mercure
 * via `MercureDomainEventHandler` like all other `DomainEvent` subtypes).
 *
 * **Auto-assign batching:** During auto-assign (§9), this method is called
 * once per task. To avoid flooding Mercure with N individual events, the
 * auto-assign service SHOULD call `Schedule::suppressEvents()` before the
 * loop and `Schedule::releaseEvents()` after, emitting a single
 * `OperatorAutoAssignCompleted` summary event instead. The SSE endpoint
 * already provides real-time progress to the initiating client; the
 * summary event notifies OTHER connected clients to refetch the snapshot.
 * For the manual assignment endpoint (`PUT /tasks/{taskId}/operator`),
 * the event fires immediately as usual.
 *
 ...
 *
 * @return list<Operator> Filtered only — full ranking (4 tiebreakers) applied by caller (see §9 auto-assign algorithm)
 */
...
export interface OperatorAbsence {
  ...
  /** Type of absence */
  type: AbsenceType;
  /** Optional reason. Always present in JSON (PHP emits the key with null value). */
  reason: string | null;
}
...
```php
/**
 * Partial update with sentinel flags — same pattern as UpdateStationRequest.
...
class UpdateOperatorRequest
{
    ...
    /** Explicit null clears the user link. Omitting the field leaves it unchanged. */
    public mixed $userId = '__SENTINEL__';  // sentinel: omitted = no change, null = clear, string = set

    /** Explicit null clears the schedule. Omitting the field leaves it unchanged. */
    public mixed $operatingSchedule = '__SENTINEL__';
}
```
 ...
 * Cross-field constraint (endDate >= startDate) enforced via #[Assert\Callback]
 * when either date field is provided.
 */
class UpdateAbsenceRequest
{
    ...
    /** Explicit null clears reason. Omitting the field leaves it unchanged. */
    public mixed $reason = '__SENTINEL__';
}
```
...
| Method | Endpoint | Input | Output | Notes |
|--------|----------|-------|--------|-------|
| `POST` | `/api/v1/schedule/auto-assign-operators` | `{scope?: 'all' \| 'unassigned'}` | SSE stream | Default: `unassigned` only. Returns **409 Conflict** if the `SseOperationLock` is held. In MVP, only auto-assign acquires this lock; detection of concurrent autoplace/smart-compaction requires optional lock retrofitting (see implementation plan). |
...
| **BR-OPER-014** | Hard | Inactive operators cannot be assigned to tasks. Enforcement: `OperatorAvailabilityConflict` with reason `'inactive'` in the TypeScript validator; auto-assign `getAvailableOperators()` filters out inactive operators. |
...
AUTO_ASSIGN_OPERATORS(schedule, operators, context, scope):
  ...
  //    Precondition: all targets must have non-null scheduledEnd (guaranteed by
  //    EndTimeCalculator at placement time). Tasks with null scheduledEnd are
  //    excluded — they lack sufficient data for operator assignment.
  targets = schedule.getInternalAssignments()
    .filter(a => a.operatorId === null && !a.isCompleted && a.scheduledEnd !== null)
    .sortBy(a => (a.scheduledStart, a.taskId))  // secondary key guarantees determinism for same-start tasks
  ...
  assigned = 0
  conflicts = []
  skipped = schedule.getAssignments().length - targets.length  // scope=unassigned: outsourced + completed + already-assigned + null-scheduledEnd; scope=all (after clearing): outsourced + completed + null-scheduledEnd
  ...
  for each assignment in targets:
    ...
    candidates = operators
      ...
      // overlaps(timeline, start, end): true if ANY entry e in timeline satisfies
      //   e.start < end AND e.end > start   (half-open intervals — adjacent tasks
      //   where e.end == start do NOT overlap, allowing back-to-back assignments)
  ...
  return { assigned, conflicts, skipped }
```

### Implementation Notes

- `isAbsentDuring(start, end)` uses **range-overlap logic**: `absence.startDate <= task.endDate AND absence.endDate >= task.startDate`. Since absences use DATE type (day-level granularity) while task times are DATETIME, both sides must be normalized to date-only values (`Y-m-d` format) before comparison — following the same pattern as `coversDate()` in §3.3. This correctly detects absences that fall anywhere within a multi-day task span, not just on boundary dates.
...
- **DST transitions:** PHP's `DateTime::setTimezone()` handles DST transitions silently (spring forward: non-existent local times are adjusted forward; fall back: ambiguous local times resolve to the first occurrence). No special handling is required in the algorithm, but unit tests should include a test vector with a task spanning a DST transition boundary (e.g., last Sunday of March in `Europe/Paris`) to verify correct behavior of `isWorkingDuring()` time-slot containment. The shared availability rules specification (`docs/architecture/operator-availability-rules.md`) should document the expected DST behavior and include corresponding cross-language test vectors.
...
### 11.1b. Auto-Assign in `scheduleApi.ts`

```typescript
// Schedule-level SSE operation (same slice as autoplace, smart compaction):
...
| `OperatorTimeline` | Vertical operator columns with assignment timeline and absence/availability overlays | OP-11 |
...
**MVP scope:** The warning header and UI toast are **deferred to post-MVP** (not assigned to any OP release). In MVP, discovery relies solely on the validation pipeline. No automatic reassignment or invalidation occurs.

---

## 12. Database Schema

The canonical DDL for all three tables is defined alongside the entity specifications:
- `operators` table — see §3.1 (`Operator` entity, Doctrine ORM annotations)
- `operator_skills` table — see §3.2 (`OperatorSkill` entity)
- `operator_absences` table — see §3.3 (`OperatorAbsence` entity)
...
| **Ephemeral auto-assign conflicts have no discovery path** | Tasks that couldn't get an operator have `operatorId=null`, so operator validators skip them (BR-OPER-008). There's no way to discover which tasks *need* operators but couldn't get one without re-running auto-assign. | Re-run auto-assign to see conflicts; V2d adds `needsOperator` flag for persistent discoverability |

...
4. [Release Breakdown](#4-release-breakdown)
5. [Phase 1: Backend Foundation](#5-phase-1-backend-foundation)
6. [Phase 2: Validation & Algorithm](#6-phase-2-validation--algorithm)
7. [Phase 3: Types Package](#7-phase-3-types-package)
8. [Phase 4: Frontend — Admin](#8-phase-4-frontend--admin)
9. [Phase 5: Frontend — Schedule Integration](#9-phase-5-frontend--schedule-integration)
10. [Phase 6: Frontend — Operator Timeline](#10-phase-6-frontend--operator-timeline)
...
OP-01  Operator Entity + Enums + Repository + CRU
  ...
  │      │
  │      ├──► OP-03  TaskAssignment.operatorId + Assignment Endpoints + Migration
  │      │      │
  │      │      └──► OP-06  @flux/types + OperatorBadge
  │      │             │
  │      │             ├──► OP-04  Operator Validation (3 conflict types)  ← imports types from OP-06
  │      │             │      │
  │      │             │      └──► OP-05  Auto-Assign Algorithm + SSE Endpoint
  │      │             │
  │      │             ├──► OP-07  ScheduleSnapshot Integration + Tile Badges
  │      │             │      │
  │      │             │      ├──► OP-09  Manual Operator Assignment UI
  │      │             │      │      │
  │      │             │      │      └──► OP-10  Auto-Assign Modal (SSE)  ← also requires OP-05
  │      │             │      │
  │      │             │      └──► OP-11  Operator Timeline Panel
  │      │             │
  │      │             └──► OP-08  Operator Management Page (Admin)
  │      │
  │      └──► OP-12  Domain Model + Business Rules Docs Update  ← independent: can start after OP-02
```

**Parallelizable work:**
- OP-08 (Admin UI) can start after OP-06. OP-07 (Tile badges) needs both OP-06 AND OP-04 (SnapshotBuilder). So OP-08 can start earlier than OP-07 if OP-04 is still in progress. `OperatorBadge` is created in OP-06 so both can import it.
- OP-12 (Docs) can start after OP-02 and run alongside any other work
- OP-06 can start as soon as OP-03 is done, in parallel with OP-04/OP-05
- OP-04 depends on OP-06 for operator types (validators import from `@flux/types`), so **OP-04 and OP-06 cannot run in parallel** — OP-06 must complete first

---

## 4. Release Breakdown

| Release | Title | Layer | Estimated Effort | Prerequisites |
|---------|-------|-------|-----------------|---------------|
| **OP-01** | Operator Entity + CRU | Backend | 1-2 days | None |
| **OP-02** | Skills + Absences | Backend | 1-2 days | OP-01 |
| **OP-03** | TaskAssignment.operatorId | Backend | 1 day | OP-02 |
| **OP-04** | Operator Validation | Validator (TS) + Backend | 1-2 days | OP-06, OP-03 |
| **OP-05** | Auto-Assign Algorithm | Backend | 3-5 days | OP-04 |
| **OP-06** | @flux/types + OperatorBadge | Types + UI | 1 day | OP-03 |
| **OP-07** | Snapshot + Tile Badges | Frontend | 1-2 days | OP-06, OP-04 |
| **OP-08** | Operator Management Page | Frontend | 2-3 days | OP-06 |
| **OP-09** | Manual Operator Assignment | Frontend | 1-2 days | OP-07 |
| **OP-10** | Auto-Assign Modal | Frontend | 1 day | OP-05, OP-09 |
| **OP-11** | Operator Timeline Panel | Frontend | 2-3 days | OP-07 |
| **OP-12** | Documentation Update | Docs | 1 day | OP-02 (minimum; can start early and iterate as later OPs complete) |

**Total estimated effort:** 16-25 person-days (sum of all releases).

**Critical path (two branches converging at OP-10):**
- **Backend branch:** OP-01 → OP-02 → OP-03 → OP-06 → OP-04 → OP-05 ≈ 7-12 days
- **Frontend branch:** (after OP-04) → OP-07 → OP-09 ≈ 2-4 days
- **Convergence:** OP-10 requires both OP-05 and OP-09 → +1 day
- **Wall-clock estimate:** ≈ 8-13 days (with OP-08, OP-11, OP-12 running in parallel off their respective prerequisites)
...
#### Prerequisites
- No release dependencies (fully independent of existing releases)
- PHP `intl` extension (required by Symfony and used by `Transliterator` for initials generation). Verify availability in local dev environments and CI.
...
- CRU controller: `OperatorController` (Create, Read, Update — **no DELETE**; operators use `status=Inactive` to decommission, following the Station pattern)
...
- BR-OPER-001: Unique name
- BR-OPER-002: Operating schedule required (validated at API level, nullable in DB for creation flow)
...
- Operator entity methods: `hasSkillFor()`, `isPrimaryFor()`, `addSkill()`, `removeSkillFor()`, `isAbsentOn()`, `addAbsence()`, `removeAbsence()`
...
#### Business Rules Implemented
- BR-OPER-010: At most one isPrimary per operator
- BR-OPER-011: No duplicate station skills
- BR-OPER-012: No overlapping absences
...
| `OperatorTest` (extended) | ~17 | `hasSkillFor()`, `isPrimaryFor()`, `addSkill()` with primary auto-clear (second primary clears first, not reject), `addSkill()` duplicate station throws, `removeSkillFor()`, `isAbsentOn()`, `addAbsence()` overlap detection, **`addAbsence()` boundary overlap (A ends March 15, B starts March 15 → rejected since both dates inclusive)**, `removeAbsence()` |
...
#### Prerequisites
- OP-02 (OperatorSkill exists for validation reference)
...
- SnapshotBuilder changes (OP-07, backend side handled there)
...
│   ├── Entity/Schedule.php                (propagate operatorId in assignTask, rescheduleTask)
...
#### Business Rules Implemented
- BR-OPER-009: Outsourced tasks throw on non-null operatorId
- BR-OPER-008: operatorId is optional (null allowed)
...
| `ScheduleTest` (extended) | ~8 | `assignTask()` with operatorId persists, `rescheduleTask()` preserves operatorId, operator assign/unassign via schedule methods |
...
## 6. Phase 2: Validation & Algorithm
...
#### Prerequisites
- OP-06 (operator types in `@flux/types` — validators import these types)
- OP-03 (TaskAssignment.operatorId exists)
- OP-01 + OP-02 (Operator, OperatorSkill, OperatorAbsence entities exist for SnapshotBuilder to query)
...
#### Business Rules Implemented
- BR-OPER-003: Skill check
- BR-OPER-004: No overlapping operator assignments
- BR-OPER-005: Absence check
- BR-OPER-006: Operating hours check

#### Test Requirements

**Unit Tests (~30 tests, TypeScript):**
...
#### Prerequisites
- OP-04 (Validation exists for result verification)
...
```
Input:
  - schedule: Schedule (all current assignments)
  - operators: Operator[] (with skills and absences loaded)
  - scope: 'all' | 'unassigned'

Output:
  - assigned: int (count of successful assignments)
  - conflicts: ConflictReport[] (tasks with no available operator)
  - skipped: int (outsourced or already assigned when scope=unassigned)

Algorithm: See operator-data-model-mvp.md § Auto-Assign Algorithm

Ranking tiebreakers:
  1. isPrimaryFor(stationId) → true before false
  2. skills.count → ascending (specialists first)
  3. assignmentsByOperator[id].count → ascending (least loaded)
```
...
```
event: progress
data: {"phase":"assigning","current":1,"total":45,"taskId":"...","stationName":"Komori G37","operatorName":"Jean Dupont"}
...
#### Prerequisites
- OP-03 (TaskAssignment.operatorId finalized)
...
#### Prerequisites
- OP-06 (@flux/types has Operator types)
- OP-01 + OP-02 (Backend CRU endpoints exist)
...
## 9. Phase 5: Frontend — Schedule Integration
...
> **Note:** This is a cross-layer release (backend SnapshotBuilder + frontend Tile changes). The backend change (SnapshotBuilder) and frontend change (Tile badges) are delivered as a single PR because the frontend change has no value without the backend data. If review scope is a concern, structure as two commits within the same PR.

#### Prerequisites
- OP-06 (@flux/types has Operator types)
- Backend: OP-04 (SnapshotBuilder updated to include operators)
...
#### Prerequisites
- OP-07 (Tile badges visible, operatorMap available)
...
│   └── ContextMenu/OperatorPicker.tsx (new: dropdown with operator list)
├── store/api/
...
#### Prerequisites
- OP-05 (Backend SSE endpoint exists)
- OP-09 (Manual assignment works — verifies plumbing)
...
## 10. Phase 6: Frontend — Operator Timeline
...
#### Prerequisites
- OP-07 (Snapshot has operators, tiles have badges)
...
### OP-12 — Documentation Update (can run in parallel with Phase 4+)
...
| `docs/domain-model/business-rules.md` | Add BR-OPER-001 through BR-OPER-012 |
...
- **OP-01 through OP-05 (Backend):** Revert migration (`bin/console doctrine:migrations:migrate prev`), revert code. TaskAssignment.operatorId null-safe — no data corruption.
...
**Rollback order:** Multi-release rollback must proceed in **reverse dependency order** to avoid compilation/runtime errors: OP-11, OP-10, OP-09, OP-08, OP-07, OP-06, OP-05, OP-04, OP-03, OP-02, OP-01, OP-12. In particular, OP-06 (types) must not be reverted before OP-07/OP-08/OP-04 (its consumers).
